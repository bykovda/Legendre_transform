...
const bool cf3D = !((fun_id == 10) || (fun_id == 2) || (fun_id == 3)); // 3D cost function
	...
	for (int i = 0; i < n1; i++)
	{
		const __m512d _x1 = _mm512_set1_pd(xs1[i]);
		const __m512d _y1 = _mm512_set1_pd(ys1[i]);
		const __m512d _z1 = cf3D ? _mm512_set1_pd(zs1[i]) : __m512d();

		__m512d _best_val = _mm512_set1_pd(to_max ? -inf : inf);
		__m512i _best_block = _mm512_set1_epi64(-1);

		const __m512d _minf = _mm512_set1_pd(-inf);
		const __m512d zeros = _mm512_setzero_pd();

		for (int jb = 0; jb < n_blocks; jb++)
		{
			int j = jb * simd_block;
			const __m512d _x2 = _mm512_load_pd(&xs2[j]);
			const __m512d _y2 = _mm512_load_pd(&ys2[j]);
			const __m512d _z2 = cf3D ? _mm512_load_pd(&zs2[j]) : __m512d();
			__m512d res;

			#include "cost_fun_avx512.cpp"

			const __m512d _g = _mm512_load_pd(&gs2[j]);
			if (fun_id == 101)
				res = _mm512_div_pd(_g, res);
			else
				res = _mm512_sub_pd(_g, res);

			const __mmask8 msk2 = _mm512_cmplt_pd_mask(_best_val, res); // best_val < res 
			if (to_max)
			{
				_best_val = _mm512_mask_blend_pd(msk2, _best_val, res);
				_best_block = _mm512_mask_blend_epi64(msk2, _best_block, _mm512_set1_epi64(jb));
			}
			else
			{
				_best_val = _mm512_mask_blend_pd(msk2, res, _best_val);
				_best_block = _mm512_mask_blend_epi64(msk2, _mm512_set1_epi64(jb), _best_block);
			}
		}

		fp* As = (fp*)_mm_malloc(simd_block  * sizeof(fp), 64);
		long long int* Is = (long long int*)_mm_malloc(simd_block  * sizeof(long long int), 64);
		_mm512_store_pd(As, _best_val);
		_mm512_store_epi64(Is, _best_block);
